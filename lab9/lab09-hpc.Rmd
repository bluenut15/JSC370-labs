...
- Cross-validation in machine learning, may use the "caret" package for parallel cross-validation with `doParallel`, or the "mlr" package with `for each` and `doParallel`.

- Bootstrapping, may use the "boot" package for bootstrapping or the "parallel" package for paralleling re-sampling.

- Markov chain Monte Carlo, may use the "parallel" package, the "rstan" package for Bayesian modelling.
...
Based on the output on their median runtime, fun1alt(100) is approximately 10 times faster than fun1(100).
...
The parallel version provides a significant speedup over the non-parallel version, as the time elapsed time for it is approximately 0.6 to 1 unit less on each run.
